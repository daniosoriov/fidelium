<?php global $smof_data, $header_setings, $post;    $data_parallax = $c_pageID = null;    if($post){        $c_pageID = $post->ID;    }    
	...
	/** data parallax */    
	if( !empty($smof_data['header_bg_parallax']) && $smof_data['header_bg_parallax'] && !empty($smof_data['background-header']['media'])){        
	...
	<div class="header-wrapper header-v1">
		<?php if ( !empty($smof_data['header_top_widgets']) && $smof_data['header_top_widgets'] =='1' && (is_active_sidebar('cshero-header-top-widget-1') || is_active_sidebar('cshero-header-top-widget-2'))): ?>    
		...
		<div id="cshero-header" class="stripe-parallax-bg <?php if($smof_data['header_fixed_top']){ echo ' transparentFixed header-'.$smof_data['header_position'].'';} ?>"<?php echo $data_parallax; ?>>         
			<?php if ( !empty($smof_data['header_top2_widgets']) && $smof_data['header_top2_widgets'] == '1' && (is_active_sidebar('cshero-header-top2-widget-1') || is_active_sidebar('cshero-header-top2-widget-2'))): ?>            
				...
				<div class="row">                
					<div class="logo logo-line-height-nav col-xs-6 col-sm-6 col-md-3 col-lg-3 <?php echo 'text-'.$smof_data["logo_alignment"];?>">                    
						<a href="<?php echo esc_url(home_url()); ?>">   
                            <?php if( !empty($smof_data['logo']['url'])){ ?>             
							<img src="<?php echo esc_url($smof_data['logo']['url']); ?>" alt="<?php esc_attr(bloginfo('name')); ?>" class="normal-logo logo-v1"/>   
                            <?php }else{ ?>
                                <img src="<?php echo esc_url(get_template_directory_uri().'/images/logo.png'); ?>" alt="<?php esc_attr(bloginfo('name')); ?>" class="normal-logo logo-v1"/>   
                            <?php } ?>                                             
						</a>                
					</div>                
					<div id="menu" class=" main-menu-wrap col-xs-6 col-sm-6 col-md-9 col-lg-9">                    
						<div class="cs-main-menu-wrap clearfix">                        
							...
							<?php if( !empty($smof_data['enable_hidden_sidebar']) && $smof_data['enable_hidden_sidebar'] =='1'){ ?>
							...
							<?php } ?>                        
							<?php if( !empty($smof_data['header_content_widgets']) && $smof_data['header_content_widgets'] =='1' && $smof_data['header_content_widgets1'] =='1' && is_active_sidebar('cshero-header-content-widget-1')){ ?>                            
							...
							<?php } ?>                         
							<?php if( !empty($smof_data['header_content_widgets']) && $smof_data['header_content_widgets'] =='1' && $smof_data['header_content_widgets2'] =='1' && is_active_sidebar('cshero-header-content-widget-2')){ ?>                            
...
<?php if( !empty($smof_data['header_sticky']) && $smof_data['header_sticky']){ get_template_part('framework/headers/sticky-header');} ?>