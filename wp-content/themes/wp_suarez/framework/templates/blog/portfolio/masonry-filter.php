...
if ($masonry_filter && count($categories) > 0) {	?>
		...
		<ul class="portfolio-filter list-unstyled list-inline col-xs-12">
			<li class=""><?php _e('Sort by: ', 'wp_suarez'); ?></li>
			<li class="filter active" data-filter="*"><span><?php _e('All', 'wp_suarez'); ?></span></li>
			<?php foreach ($categories as $category) { if(!empty($category->slug)){ ?>
