        ...
        <div class="cshero-portfolio-item-content">
            ...
            <div class="cshero-portfolio-content-wrap-inner">
                ...
                <div class="link-wrap <?php echo esc_attr($link_type); ?>">
                    <?php
                    if($link_type == 'button'){
                        $launch_link = '<a class="btn btn-default" href="' . esc_url($portfolio_link) . '" target="_blank">'.__($link_text,'wp_suarez').'</a>';
                        $readmore_link = '<a class="btn btn-default" title="" href="' . esc_url(get_the_permalink())  . '" >'.__($read_more_text,'wp_suarez').'</a>';
                        $zoom_link = '<a class="btn btn-default colorbox" href="'.esc_url($full_image).'" >'.__($zoom_text,'wp_suarez').'</a>';
                    } elseif($link_type == 'text'){
                        $launch_link = '<a class="" href="' . esc_url($portfolio_link) . '" target="_blank">'.__($link_text,'wp_suarez').'</a>';
                        $readmore_link = '<a class="" title="" href="' . esc_url(get_the_permalink())  . '" >'.__($read_more_text,'wp_suarez').'</a>';
                        $zoom_link = '<a class="colorbox" href="'.esc_url($full_image).'" >'.__($zoom_text,'wp_suarez').'</a>';
                    } else {
                        $launch_link = '<a class="icon-link" href="' . esc_url($portfolio_link) . '" title="'.__(get_the_title(),'wp_suarez').'" target="_blank"><i class="'.$link_icon.'"></i></a>';
                        $readmore_link = '<a class="icon-link" title="'.__(get_the_title(),'wp_suarez').'" href="' . esc_url(get_the_permalink())  . '" ><i class="'.$read_more_icon.'"></i></a>';
                        $zoom_link = '<a class="icon-link colorbox" title="'.__(get_the_title(),'wp_suarez').'" href="'.esc_url($full_image).'" ><i class="'.$zoom_icon.'"></i></a>';
                    }
