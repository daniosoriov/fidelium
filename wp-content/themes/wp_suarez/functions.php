...
load_theme_textdomain( 'wp_suarez', get_template_directory().'/languages');
// Register Navigation
...
function cshero_register_menu(){
    register_nav_menu('main_navigation', __('Main Navigation','wp_suarez'));
    register_nav_menu('top_navigation', __('Top Navigation','wp_suarez'));
    register_nav_menu('left_navigation', __('Left Navigation','wp_suarez'));
    register_nav_menu('right_navigation', __('Right Navigation','wp_suarez'));
    register_nav_menu('404_pages', __('404 Useful Pages','wp_suarez'));
    register_nav_menu('sticky_navigation', __('Sticky Header Navigation','wp_suarez'));
}
 ...
 * @author FOX
 */
add_action('vc_after_init', 'wp_suarez_vc_after');

function wp_suarez_vc_after() {
    require( get_template_directory() . '/vc_params/vc_tta.php' );
...
function cshero_css(){
	...
	if ( !empty($smof_data["use_font_awesome"]) && $smof_data["use_font_awesome"] == '1') {
		wp_enqueue_style( 'font-awesome' );
	}

	if ( !empty($smof_data["use_font_ionicons"]) && $smof_data["use_font_ionicons"] == '1') {
...
function cshero_js(){
	...
	if( !empty($smof_data["smooth_scroll"]) && $smof_data['smooth_scroll'] == '1'){
    ...
    $show_sticky_header = !empty($smof_data["header_sticky"]) ? $smof_data['header_sticky'] : '0';
    if(is_page()){
    ...
    if( !empty($smof_data["page_loader"]) && $smof_data['page_loader'] == '1'){
...
function cs_socials_share($url,$image='',$title='',$comment_link=''){
	...
	<div class="post-share">
		...
		<a target="_blank" href="https://twitter.com/home?status=<?php echo urlencode(__('Check out this article','wp_suarez'));?>:%20<?php echo urlencode($title);?>%20-%20<?php echo urlencode($url);?>"><span class="share-box"><i class="fa fa-twitter"></i></span></a>
...
function cshero_footer(){
	global $smof_data;
    $footer_layout = !empty($smof_data["footer_layout"]) ? $smof_data["footer_layout"] : 'f1';
...
if(!function_exists('cshero_generetor_blog_layout')){
	function cshero_generetor_blog_layout() {
		...
		$category_layout = !empty($smof_data['blog_layout']) ? $smof_data['blog_layout'] : 'left-fixed' ;
...
if(!function_exists('cshero_generetor_header_setting')){
	function cshero_generetor_header_setting(){
		...
		if(is_page() && get_post_meta($post->ID, 'cs_header_setting', true) == 'custom'){
			...
			$header_setings->header_fixed = ( !empty($smof_data['header_fixed_top']) && $smof_data['header_fixed_top'] == '1') ? $smof_data['header_fixed_top'] : '0';
		...
		if(!empty($smof_data["header_top2_padding"])){
...
if(!function_exists('cshero_posts_full_content')){
	function cshero_posts_full_content(){
		global $smof_data;
		if( !empty($smof_data['blog_full_content']) && $smof_data['blog_full_content'] == '1'){
			return '1';
		} else {
			return '0';
		}
...
function cshero_breadcrumb() {
    ...
    if(class_exists('bbPress') && is_bbpress()){
        ...
        if( is_search() ){ echo '<li>'.__("Search", 'wp_suarez').'</li>'; }
        if( is_year() ){ echo '<li>'.get_the_time('Y').'</li>'; }
        
        if( is_404() ) {
            echo '<li>'.__("404 - Page not Found", 'wp_suarez').'</li>';
        }        
...
function cshero_page_title_render() {
    ...
    if (is_page() && get_post_meta(get_the_ID(), 'cs_page_title_custom_text', true) != ""){
        ...
        if (!is_archive()){
            if(is_search()){
                printf( __( 'Search Results for: %s', 'wp_suarez' ), '<span>' . get_search_query() . '</span>' );
            } elseif (is_404()){
                _e( '404', 'wp_suarez');
            } else {
            ...
            single_tag_title();
            elseif ( is_author() ) :
            printf( __( 'Author: %s', 'wp_suarez' ), '<span class="vcard">' . get_the_author() . '</span>' );
            elseif ( is_day() ) :
            printf( __( 'Day: %s', 'wp_suarez' ), '<span>' . get_the_date() . '</span>' );
            elseif ( is_month() ) :
            printf( __( 'Month: %s', 'wp_suarez' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'wp_suarez' ) ) . '</span>' );
            elseif ( is_year() ) :
            printf( __( 'Year: %s', 'wp_suarez' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'wp_suarez' ) ) . '</span>' );
            elseif ( is_tax( 'post_format', 'post-format-aside' ) ) :
            _e( 'Asides', 'wp_suarez' );
            elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) :
            _e( 'Galleries', 'wp_suarez');
    ...
    function cshero_404_page_default(){
        ...
        ?>
         <div class="col-xs-12 col-sm-6 col-md-6 col-lg-6 error_image">
    		   <img alt="" src="<?php if($smof_data['404_image']){ echo esc_url($smof_data['404_image']['url']); } else { echo esc_url($smof_data['logo']); } ?>">
    		</div> 
		<div class="row">
			<div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 error_content">
			   <div class="error-body">
			   		<h1><?php _e('404 Page not found', 'wp_suarez'); ?></h1>
			       <span><?php _e('Whoops, sorry, this page does not exist.', 'wp_suarez'); ?></span>
			   </div>
			</div>
		</div>
        <?php
...
function wp_suarez_inline_style( $css_includes ) {  
    if ( !empty($css_includes) ) {  
        wp_enqueue_style('suarez-inline-style',get_template_directory_uri() . '/css/suarez-inline-style.css');
        wp_add_inline_style('suarez-inline-style', $css_includes);
    }
}
...
add_action('after_setup_theme','wp_suarez_update_woo_number_item_in_page');
function wp_suarez_update_woo_number_item_in_page(){
    global $smof_data;
    if(class_exists('EF3_Framework')){
        if(class_exists( 'woocommerce' )){
            $number_product = ( !empty($smof_data['woo_number_products']) ) ? $smof_data['woo_number_products'] : 16;
            add_filter( 'loop_shop_per_page', create_function( '$cols', 'return '.$number_product.';' ), 16 );
        }
    }
}
...
?>
	...
	<div class="comment-meta-wrap">
		...
		<?php if ( $comment->comment_approved == '0' ) : ?>
			<em class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.','wp_suarez' ); ?></em>
			<br />
		<?php endif; ?>
		...
		<div class="comment-meta commentmetadata">
			<a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>">
				...
				<?php edit_comment_link( __( '(Edit)','wp_suarez' ), '  ', '' );
			?>
			<div class="reply">
